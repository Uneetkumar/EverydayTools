        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 @md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <span className="block text-xs font-semibold text-slate-700 dark:text-slate-300">Position</span>
                  ...
                  ([id, label]) => (
                    <button key={id} onClick={() => setPosition(id)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                        position === id ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300"
                      }`}>{label}</button>
                  )
                )}
              </div>
            ...
            <div className="space-y-1.5">
              <label htmlFor="startAt" className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
                Start numbering at
              </label>
              <input id="startAt" type="number" min={0} value={startAt}
                onChange={(e) => setStartAt(parseInt(e.target.value || "1", 10))}
                className="w-28 text-sm font-mono bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-3 py-2 rounded-xl text-slate-900 dark:text-white" />
            </div>
          </div>
