...
export default function VideoMetadataDisplay({ metadata }: VideoMetadataProps) {
  const specs = [
    ...
    },
    {
      ...
      subvalue: metadata.bitrateKbps ? `${metadata.bitrateKbps} kbps avg` : "Adaptive stream",
      icon: Activity,
    },
    {
      label: "Aspect Ratio",
    ...
    },
    {
      ...
      subvalue: "Stereo (2.0 channels)",
      icon: Volume2,
    },
    {
      label: "FPS / Refresh",
      value: `${metadata.approxFps} fps`,
      subvalue: "Progressive scan",
      icon: Layers,
    },
  ];
  ...
  return (
    <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-mono text-slate-400 truncate max-w-[200px]" title={metadata.fileName}>
          {metadata.fileName}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {specs.map((spec) => {
          ...
          return (
            <div
              key={spec.label}
              className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800/80 space-y-1.5 transition hover:border-blue-500/40"
            >
